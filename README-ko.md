...
> **AI 에이전트의 의도치 않은 시스템 조작 가능성을 구조적으로 줄이기 위해 만든 단일 스크립트 기반 K3s 개발 환경**

환경: Oracle Cloud Always-Free ARM 인스턴스 (4 OCPU, 24GB RAM) + Synology NAS (백업/스토리지)
목표: VPN 없이 퍼블릭 네트워크에서 운영 가능한 K3s 인프라를 구축하고, LLM 코딩 에이전트에게는 인증·권한이 제한된 경로를 통해 조건부 접근만 허용하는 것.
